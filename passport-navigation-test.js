...
// For now, test on one device only
...
(async () => {
  ...
  try {
    ...
    if (await errorMessage.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      testResults.validation_eth = 'pass';
    } else {
      console.log('⚠ No validation error detected');
      testResults.validation_eth = 'fail';
    }
    ...
    if (await errorMessage.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      console.log(`   Error message: "${errorText}"`);
      await page.waitForTimeout(3000);
      testResults.validation_ens = 'pass';
    } else {
      console.log('⚠ No validation error detected');
      testResults.validation_ens = 'fail';
    }
    ...
    testResults.collection = clicked ? 'pass' : 'fail';
    ...
    if (await benefitsNav.isVisible({ timeout: 10000 }).catch(() => false)) {
      ...
      testResults.benefits = clicked ? 'pass' : 'fail';
    }
    ...
    if (await scanButton.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      await page.screenshot({ path: './passport-screenshots/scan-interface.png', fullPage: true });
    ...
    if (latestVideo) {
      const newVideoName = `passport-test-${formattedDate}.webm`;
      await fs.rename(
